...
def create_art(instance):
    DEBUG = True

    image_one_path = instance.image_one.path
    content_image = load_image(image_one_path, max_size=None)

    image_two_path = instance.image_two.path
    style_image = load_image(image_two_path, max_size=300)
    ...
    instance.image_output.save(image_name, image_file)

    connection.close()
    return instance.image_output
    ...
    def post(self, request):
        if "user_email" in request.session:
            ...
            if form.is_valid():
                ...
                art_generator = threading.Thread(target=create_art, args=[instance])
